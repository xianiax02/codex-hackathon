...
이주민 부모가 내일 담임 선생님에게 할 말을 입력하고, AI 선생님과 한 번 말해 본 뒤, 전달을 막은
표현과 소리 하나를 고쳐 다시 말하고, 바로 쓸 세 문장을 가져간다.

성공 기준은 한국어 실력 평가가 아니라 **학교 결석 연락을 혼자 끝낼 준비가 되었는가**다.
...
| demo 장면 | 아이의 당일 결석을 담임에게 알리기 | 60초 안에 목적과 감정이 설명된다 |
...
|---:|---|---|---|
| 0–7초 | `내일 선생님에게 민수가 아파서 결석한다고 말해야 해요` 입력 | 상대·목적·필수 정보가 생성됨 | LLM이 생활 목표를 rehearsal로 바꾼다 |
| 7–15초 | `연습 시작` | 담임 선생님이 실제 질문 하나를 함 | 고정 교재가 아닌 role-play다 |
| 15–29초 | microphone으로 첫 답변 | 발화 transcript와 대화의 다음 반응이 나타남 | 사용자가 자기 목소리로 연습한다 |
| 29–39초 | 피드백 확인 | 쉬운 표현 1개와 잘 전달되지 않은 단어 1개만 표시 | 실제 발화에 맞춰 우선순위를 고른다 |
| 39–52초 | 짧아진 목표 문장을 다시 말함 | before/after transcript가 나란히 표시됨 | 피드백을 즉시 적용한다 |
| 52–60초 | 완료 | 내일 쓸 세 문장과 예상 질문 하나가 남음 | 학습이 실제 행동으로 끝난다 |

demo 중 AI 대화는 **질문 1회, 답변 1회, retry 1회**뿐이다. 긴 자유 대화는 만들지 않는다.
...
| 구간 | 시간 | 내용 |
|---|---:|---|
| 문제 장면 | 0:00–0:35 | 내일 담임에게 전화해야 하지만 연습할 시간과 상대가 없다 |
